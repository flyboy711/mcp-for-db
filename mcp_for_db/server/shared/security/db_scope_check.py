...
class DatabaseScopeChecker:
    ...
    SPECIAL_QUERY_PATTERNS = {
        ...
        'SYSTEM_TABLE_ACCESS': [
            r'\bserver_mysql\.user\b',
            r'\bserver_mysql\.db\b',
    ...
    def __init__(self, session_config: SessionConfigManager):
        ...
        # 获取配置项（不加前缀）
        self.is_enabled = session_config.get('MYSQL_ENABLE_DATABASE_ISOLATION', False)
        self.allowed_database = session_config.get('MYSQL_DATABASE', '')

        # 获取数据库访问级别（字符串转换为枚举）
        access_level_str = session_config.get('MYSQL_DATABASE_ACCESS_LEVEL', 'permissive').lower()
        try:
